...
func (a *APIClient) ListOfflineCollectors(localVarOptionals *types.ListCollectorsOfflineOpts) (types.ListCollectorsModel, *http.Response, error) {
	var (
		...
		localVarReturnValue types.ListCollectorsModel
	)

	// create path and map variables
	localVarPath := a.Cfg.BasePath + "/v1/collectors/offline"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}

	if localVarOptionals != nil && localVarOptionals.AliveBeforeDays.IsSet() {
		localVarQueryParams.Add("aliveBeforeDays", parameterToString(localVarOptionals.AliveBeforeDays.Value(), ""))
	}
	...
	if localVarHttpResponse.StatusCode < 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v types.ListCollectorsModel
