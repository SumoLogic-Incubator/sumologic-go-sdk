...
*/
func (a *APIClient) ImportMonitorsItem(body types.MonitorsLibraryBaseExport, parentId string) (types.MonitorsLibraryBaseResponse, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Post")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue types.MonitorsLibraryBaseResponse
	)

	// create path and map variables
	localVarPath := a.Cfg.BasePath + "/v1/monitors/{parentId}/import"
	localVarPath = strings.Replace(localVarPath, "{"+"parentId"+"}", fmt.Sprintf("%v", parentId), -1)
	...
	if localVarHttpResponse.StatusCode < 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v types.MonitorsLibraryBaseResponse
