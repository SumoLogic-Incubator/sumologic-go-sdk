...
*/
func (a *APIClient) UpdateIdentityProvider(body types.SamlIdentityProviderRequest, id string) (types.SamlIdentityProvider, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Put")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue types.SamlIdentityProvider
	)

	// create path and map variables
	localVarPath := a.Cfg.BasePath + "/v1/saml/identityProviders/{id}"
	...
	if localVarHttpResponse.StatusCode < 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v types.SamlIdentityProvider
