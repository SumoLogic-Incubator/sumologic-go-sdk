...
/*
GetLookupTableAsyncExportResult
Get lookup table results from content export job for the given job identifier. The results from this export are incompatible with the Library import feature in the Sumo user interface.
...
func (a *APIClient) GetLookupTableAsyncExportResult(contentId string, jobId string, localVarOptionals *types.ContentOpts) (types.LookupTableSyncDefinition, *http.Response, error) {
	var (
		...
		localVarReturnValue types.LookupTableSyncDefinition
	)

	// create path and map variables
	localVarPath := a.Cfg.BasePath + "/v2/content/{contentId}/export/{jobId}/result"
	...
	if localVarHttpResponse.StatusCode < 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v types.LookupTableSyncDefinition
