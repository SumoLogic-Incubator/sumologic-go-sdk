...
*/
func (a *APIClient) SetUserConcurrentSessionsLimitPolicy(body types.UserConcurrentSessionsLimitPolicy) (types.UserConcurrentSessionsLimitPolicy, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Put")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue types.UserConcurrentSessionsLimitPolicy
	)

	// create path and map variables
	localVarPath := a.Cfg.BasePath + "/v1/policies/userConcurrentSessionsLimit"
	...
	if localVarHttpResponse.StatusCode < 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v types.UserConcurrentSessionsLimitPolicy
